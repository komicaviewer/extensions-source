...
class Site2catBoardCatalogTest {

    @Test
    fun `catalog keeps the previous 2cat board identity and order`() {
        assertEquals(
            listOf(
            ...
            Site2catBoardCatalog.boards.map { it.name to it.url },
        )
        assertTrue(Site2catBoardCatalog.boards.all { it.sourceId == Site2catBoardCatalog.SOURCE_ID })
    }
}
