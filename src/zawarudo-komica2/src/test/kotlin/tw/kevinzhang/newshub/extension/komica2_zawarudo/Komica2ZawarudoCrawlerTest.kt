...
class ZawarudoCrawlerTest {
    ...
    fun `board parser maps posts paragraphs attachments and omitted replies`() {
        ...
        assertTrue(post.content.any { it is Paragraph.Text && it.content.contains("本文") })
        assertTrue(post.content.any { it == Paragraph.ReplyTo("123") })
        assertTrue(post.content.any { it == Paragraph.Quote("一般引用") })
        assertTrue(post.content.any { it == Paragraph.Link("https://example.com/page") })
        assertTrue(
            post.content.any {
                it == Paragraph.ImageInfo(
