...
class Komica2ThreadSummariesRequestBuilder : ThreadSummariesRequestBuilder {
    private lateinit var builder: HttpUrl.Builder
    private var boardUrl: HttpUrl? = null

    override fun setUrl(url: HttpUrl): Komica2ThreadSummariesRequestBuilder {
        builder = url.newBuilder()
        return this
    }

    fun setBoard(board: Board): Komica2ThreadSummariesRequestBuilder {
        boardUrl = board.url.toHttpUrl()
        return setUrl(boardUrl!!)
    }

    override fun setPage(page: Int?): Komica2ThreadSummariesRequestBuilder {
        val board = boardUrl
