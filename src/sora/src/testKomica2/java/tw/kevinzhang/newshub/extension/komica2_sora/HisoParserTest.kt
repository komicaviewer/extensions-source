...
class HisoParserTest {
    ...
    fun `summary parser groups bare thread posts without duplicates and uses lazy thumbnail`() {
        val request = Komica2SoraFactory()
            .createThreadSummariesRequestBuilder(Komica2SoraBoards.all.last())
            .setPage(1)
            .build()

        val posts = Komica2SoraFactory()
            .createThreadSummariesParser(Komica2SoraFactory().createThreadUrlParser())
        ...
        val firstImage = posts.first().content.filterIsInstance<Paragraph.ImageInfo>().single()
        ...
        assertTrue(posts.first().content.filterIsInstance<Paragraph.ImageInfo>().none { it.thumb == "/share/theme/white.png" })
    }
