...
class Komica2TwocatRequestTest {
    ...
    private val board = Komica2TwocatBoards.all[1]
    ...
    fun `page number and referer follow Komica2 Pixmicat rules`() {
        ...
        assertEquals("https://2cat.org/~hanime/pixmicat.php", firstPage.url.toString())
        assertEquals("https://2cat.org/~hanime/pixmicat.php?page_num=1", secondPage.url.toString())
        assertEquals("https://2cat.org/~hanime/pixmicat.php?page_num=2", thirdPage.url.toString())
    ...
    fun `thread request keeps pixmicat endpoint res query and referer`() {
        val request = engine.createThreadRequestBuilder(board)
            .setUrl("https://2cat.org/~hanime/pixmicat.php?res=42".toHttpUrl())
            .build()

        assertEquals("https://2cat.org/~hanime/pixmicat.php?res=42", request.url.toString())
