...
/** HTML metadata not represented by extension-api's final Post model. */
...
internal class ParsedPostBuilder {
    ...
    fun setPoster(value: String) = apply { poster = value }
    fun setCreatedAt(value: Long) = apply { createdAt = value }
    fun setContent(value: List<Paragraph>) = apply { content = value }
    fun addContent(value: Paragraph) = apply { content = content + value }
    fun setUrl(value: String) = apply { url = value }
    fun setPostId(value: String) = apply { id = value }
    fun build() = ParsedPost(id, url, title, createdAt, poster, content = content)
}

internal fun List<ParsedPost>.replyCountFor(postId: String): Int = count { post ->
